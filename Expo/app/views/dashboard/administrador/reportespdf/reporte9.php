...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,'Listado de codigos disponibles ',0,0,'C',false);  
    $this->Ln();  
}
// Page footer
function Footer()
...
$fpdf->setTitle('Codigos Disponibles');
...
session_start();
$codigo= new codigo();  
$data=$codigo->getCodigosEspecial0();
$fpdf->AddPage();  
$fpdf->titulo();   
if($data!=null){ 
$fpdf->Ln(1);
$jeje=null;
foreach($data as $row){
$header = array($row['tipo']);
$fpdf->Cell(0.5); 
$fpdf->tabla($header,$row);  
$fpdf->Ln(1);  
if($codigo->setIdAlumno($row['id'])){
    $data1=$codigo->getCodigosEspecial();  
    $header1=array("Codigo");
    $fpdf->Cell(0.5);
    ...
    $fpdf->Cell(0,1,'No hay codigos disponibles',0,1,'C',false);
}
