...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
     $this->Ln();     
    $curso=new Cursos();
    if($curso->setId($_POST['grados'])){ 
    if($curso->readGrados2()){ 
        $this->Cell(0,1,"Cursos disponibles en ".$curso->getGrado(),0,2,'C',false); 
       }
    }  
}
// Page footer
...
$fpdf->Ln(1);
$jeje=null; 
$header = array("Materia","Maestro que la imparte" ); 
$fpdf->Cell(0.5);
...
$fpdf->Ln(1);    
    ...
    $fpdf->Cell(0,1,'Este grado aun no tiene cursos asignados',0,1,'C',false);
}
