...
{ 
    ...
    $this->Ln();     
    $this->Cell(0,1,"Grado : ".$_SESSION['grados1'],0,2,'L',false);   
    $this->Cell(0,1,"Historial de codigos entre las fechas : ".$_SESSION['fecha1']." y ".$_SESSION['fecha2']);
    $this->Ln();
}
// Page footer
function Footer()
...
session_start();
$codigos= new Codigo(); 
if($codigos->setIdAlumno($_SESSION['grados'])){ 
     $data1=$codigos->getCodigoAlumnos3($_SESSION['fecha1'],$_SESSION['fecha2']); 
} 
$fpdf->AddPage();  
$fpdf->titulo();   
if($data1!=null){ 
...
$fpdf->Line(1.6,$fpdf->getY()+2.2,20,$fpdf->getY()+2.2);
    ...
    $fpdf->Ln(5);
    $fpdf->setFont('Arial','B',16);
    $fpdf->Cell(0,1,'Este grado aun no tiene codigos asignados en este intervalo de fechas',0,1,'C',false);
}
