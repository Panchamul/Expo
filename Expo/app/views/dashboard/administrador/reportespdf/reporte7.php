...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,'Alumnos reprobados ',0,2,'C',false);  
...
{ 
    ...
    {      
        $this->Cell(0.5);
         $this->Cell($w[0],1.6,$row[0]." ".$row[1],'LR',0,'C',$fill);    
         $this->Cell($w[1],1.6,$row[2],'LR',0,'C',$fill); 
...
$fpdf->setTitle('Alumnos reprobados');
...
$fpdf->tabla($header);  
$fpdf->Ln(1); 
    $header1=array("Alumno","Grado","Promedio global");
    $fpdf->Cell(0.5);
...
if($contador<1){
    ...
    $fpdf->Cell(0,1,'No hay alumnos reprobados en este periodo',0,1,'C',false);
}
}
