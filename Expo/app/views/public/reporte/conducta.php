...
{ 
    ...
    $this->Cell(0,0.9,'Nombre: '.$_SESSION['nombre2login']." ".$_SESSION['apellido2login'],0,0,'C',false);
    // Line break
    $this->Ln(1.5);
    }
}
...
{ 
    ...
    $this->Cell($w[$i],1,$header2[$i],1,0,'C',true);
    $this->Ln(); 
    ...
    {      
         $this->Cell($w[0],1.6,$row[1],'LR',0,'C',$fill); 
        $this->Ln();
        $fill = !$fill;
    }
    // Línea de cierre 
...
session_start();
$codigos= new Codigo(); 
if($codigos->setIdAlumno($_SESSION['id_session2'])){ 
     $data1=$codigos->getCodigoAlumnos();
     $data2=$codigos->getObservacionesVistosPorEstudiantes();
} 
$fpdf->AddPage();  
$fpdf->titulo();   
...
$fpdf->Ln(1);
$jeje=null;
 $fpdf->Cell(0,1,'Codigos Asignados',0,1,'C',false);   
$header = array("Codigo","Tipo de codigo","Fecha"); 
$fpdf->tabla1($header,$data1);  
$fpdf->Ln(1);   
$fpdf->setFont('Arial','B',16);
$fpdf->Cell(0,1,'Observaciones ',0,1,'C',false);     
$header1 = array("Observacion"); 
$fpdf->tabla2($header1,$data2);
}
else{
    $fpdf->Ln(5);
    $fpdf->setFont('Arial','B',16);
    $fpdf->Cell(0,1,'Este alumno aun no tiene codigos asignados',0,1,'C',false);
}
